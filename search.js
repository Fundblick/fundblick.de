...
(function () {
  ...
  const queryAliases=[
    ...
    [/(?:телевизор[ыа]?|телевізор[иів]?|televizyon|televisione|تلفاز|تلفزيون|telewizor|televizor|телевизор|τηλεόραση|телевизор|televisor|téléviseur|televisão|تلویزیون|televizor|电视|televîzyon)/giu,'Fernseher'],
  ...
  function features(product){
    const text=`${product.name||''} ${product.description||''}`;const found={};const put=(key,value)=>{if(value)found[key]=String(value)};
    ...
    if(product.family==='tv'){const screen=text.match(/\b(\d{2,3})\s*(?:inch(?:es)?|zoll|["″])/i);if(screen)put('screen',screen[1]+' Zoll');const panel=text.match(/\b(OLED|QLED|Mini-LED)\b/i);if(panel)put('panel',panel[1].toUpperCase())}
    if(product.family==='shoes'){const size=text.match(/(?:size|gr[oö][ßs]e|EU)\s*(3[5-9]|4[0-8])\b/i);if(size)put('size',size[1])}
    if(product.family==='phone'){const storage=text.match(/\b(64|128|256|512|1024)\s*GB\b/i);if(storage)put('storage',storage[1]+' GB')}
    if(product.family==='heatgun'){const power=text.match(/\b(\d{3,4})\s*(?:W|Watt)\b/i);if(power)put('power',power[1]+' W')}
  ...
  function normalize(raw){
    ...
    const text=`${raw.name} ${raw.description||''}`,taxonomy=String(raw.category||'');
  ...
  function interpret(query){
    ...
    if(category?.id==='tv'){const screen=clean.match(/\b(\d{2,3})\s*(?:zoll|inch(?:es)?|["″])/i);if(screen)constraints.screen=screen[1]+' Zoll';const panel=clean.match(/\b(OLED|QLED|Mini-LED)\b/i);if(panel)constraints.panel=panel[1].toUpperCase()}
    if(category?.id==='shoes'){const size=clean.match(/(?:gr[oö][ßs]e|size|EU)\s*(3[5-9]|4[0-8])\b/i);if(size)constraints.size=size[1]}
    if(category?.id==='phone'){const storage=clean.match(/\b(64|128|256|512|1024)\s*GB\b/i);if(storage)constraints.storage=storage[1]+' GB'}
    for(const [key,value] of Object.entries(constraints)){if(!state.facets[key])state.facets[key]=new Set();state.facets[key].add(value);clean=clean.replace(new RegExp(value.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'i'),' ')}
    if(category)clean=clean.replace(category.query,' ');clean=clean.replace(/\b(?:bluetooth|wireless|kabellos|kabelgebunden|wired|zoll|inch|size|gr[oö][ßs]e|EU|GB|euro)\b/gi,' ');
  ...
  function filtered(skip){return base.filter(p=>{if(state.min!==null&&p.price<state.min||state.max!==null&&p.price>state.max)return false;if(skip!=='brand'&&state.brands.size&&!state.brands.has(p.brand))return false;return Object.entries(state.facets).every(([key,set])=>skip===key||!set.size||set.has(p.attrs[key]))})}
  function optionsFor(key){const set=new Set(base.map(p=>key==='brand'?p.brand:p.attrs[key]).filter(Boolean));return [...set].sort((a,b)=>a.localeCompare(b,locale,{numeric:true}))}
  function group(key,label,values){if(!values.length)return '';const selected=key==='brand'?state.brands:state.facets[key]||new Set();return `<section class="facet"><h2>${esc(key==='brand'?tx('manufacturer'):tx(facetLabels[key]||label))}</h2>${values.map(value=>{const count=filtered(key).filter(p=>(key==='brand'?p.brand:p.attrs[key])===value).length;return `<label><input type="checkbox" data-key="${esc(key)}" value="${esc(value)}" ${selected.has(value)?'checked':''}>${esc(displayValue(value))}<span>${count}</span></label>`}).join('')}</section>`}

  function renderFilters(){
    const schema=category?.facets||[],specialized=schema.map(([key,label])=>group(key,label,optionsFor(key))).join('');
    ...
    filtersEl.querySelectorAll('input[type=checkbox]').forEach(el=>el.addEventListener('change',()=>{const set=el.dataset.key==='brand'?state.brands:(state.facets[el.dataset.key]??=new Set());el.checked?set.add(el.value):set.delete(el.value);render()}));
  ...
  function renderChips(){const chips=[];if(state.min!==null)chips.push(['min',`${tx('from')} ${money(state.min)}`]);if(state.max!==null)chips.push(['max',`${tx('to')} ${money(state.max)}`]);for(const brand of state.brands)chips.push(['brand:'+brand,`${tx('manufacturer')}: ${brand}`]);for(const [key,values] of Object.entries(state.facets))for(const value of values)chips.push([key+':'+value,displayValue(value)]);chipsEl.innerHTML=chips.map(([key,label])=>`<button type="button" data-remove="${esc(key)}" aria-label="${esc(removeWord[language.lang]||removeWord.en)}: ${esc(label)}">${esc(label)} ×</button>`).join('');chipsEl.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>{const [key,...rest]=b.dataset.remove.split(':'),value=rest.join(':');if(key==='min'||key==='max')state[key]=null;else if(key==='brand')state.brands.delete(value);else state.facets[key]?.delete(value);render()}))}
  ...
  function card(p){const tags=Object.values(p.attrs).slice(0,3).map(x=>`<span>${esc(displayValue(x))}</span>`).join('');return `<article class="product">${p.image?`<img src="${esc(p.image)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:`<div class="no-image" aria-hidden="true">${esc(tx('results'))}</div>`}<div><p><bdi>${esc(p.brand||p.category)}</bdi> · ${esc(tx('testData'))}</p><h2 dir="auto">${esc(p.name)}</h2><p dir="auto">${esc(p.description.slice(0,150))}</p><div class="tags">${tags}</div></div><div class="price"><strong>${money(p.price)}</strong><small>${esc(tx('testPrice'))}</small><span class="unavailable">${esc(tx('noOffer'))}</span></div></article>`}

  function render(){renderFilters();renderChips();let list=filtered();if(state.sort==='price-asc')list.sort((a,b)=>a.price-b.price);else if(state.sort==='price-desc')list.sort((a,b)=>b.price-a.price);else if(state.sort==='brand')list.sort((a,b)=>a.brand.localeCompare(b.brand,locale));const categoryText=categoryLabels[category?.id]?tx(categoryLabels[category.id]).replace(/^[^\p{L}\p{N}]+/u,''):'';summaryEl.textContent=`${list.length} ${list.length===1?tx('oneFound'):tx('found')}${categoryText?' · '+categoryText:''}`;cardsEl.innerHTML=list.length?list.slice(0,100).map(card).join(''):`<div class="empty"><h2>${esc(tx('noResults'))}</h2><p>${esc(tx('noResultsHelp'))}</p></div>`;writeUrl()}

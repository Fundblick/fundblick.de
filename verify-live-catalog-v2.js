...
assert.equal(manifest.offerSchema,'simulated-v1');
...
const seen=new Set();let counted=0,multiOfferProducts=0;
for(const [key,meta] of Object.entries(manifest.shards)){
  ...
  for(const product of payload){
    ...
    for(const offer of product.offers){
      ...
      assert.equal(Math.round((offer.price+offer.shippingCost)*100),Math.round(offer.totalPrice*100),`invalid total for ${product.id}`);
      assert.equal(offer.simulated,true,`live-beta offer must be marked simulated for ${product.id}`);
    }
    const bestPool=product.offers.filter(offer=>offer.inStock);const expectedBest=(bestPool.length?bestPool:product.offers).slice().sort((a,b)=>a.totalPrice-b.totalPrice||a.deliveryDays-b.deliveryDays||a.merchant.localeCompare(b.merchant))[0];
    assert.equal(product.bestOffer.id,expectedBest.id,`best offer mismatch for ${product.id}`);
    assert.equal(product.price,expectedBest.price,`display price must use best offer for ${product.id}`);
    assert.equal(product.shippingCost,expectedBest.shippingCost,`display shipping must use best offer for ${product.id}`);
  }
}
assert.equal(counted,manifest.itemCount);
assert.ok(multiOfferProducts>=10,'too few products exercise merchant comparison');
for(const row of index){assert.ok(seen.has(row.i),`index points to missing ${row.i}`);assert.ok(manifest.shards[String(row.s)],`invalid shard ${row.s}`);assert.ok(row.q.length>0);assert.ok(Number.isFinite(row.t)&&row.t>=row.p);assert.ok(Number.isInteger(row.o)&&row.o>=1);}
...
console.log('live catalog v2 + simulated merchant offer verification passed');

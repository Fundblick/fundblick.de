...
(function(){
  ...
  function current(){const params=new URLSearchParams(location.search),select=document.querySelector('#language');let saved=null;try{saved=localStorage.getItem('fundblick-language')}catch{}return normalize(select?.value)||normalize(params.get('lang'))||normalize(window.FundBlickLanguage?.lang)||normalize(document.documentElement.lang)||normalize(saved)||'de';}
  ...
  function apply(lang=current()){
    document.querySelectorAll('a[href]').forEach(anchor=>{if(!eligible(anchor))return;const url=new URL(anchor.getAttribute('href'),location.href);url.searchParams.set('lang',lang);const sameOrigin=url.origin===location.origin;if(!sameOrigin)return;anchor.href=url.pathname+url.search+url.hash;});

...
(function(){
  ...
  function initialLanguage(){
    ...
    try{const saved=normalizeLanguage(localStorage.getItem('fundblick-language'));if(saved)return saved}catch{}
    for(const candidate of navigator.languages||[navigator.language]){const match=normalizeLanguage(candidate);if(match)return match}
    return 'de';
  }
  ...
  function choose(i){if(i<0||i>=hits.length)return;input.value=hits[i].q;close();input.focus()}
  function render(){
    if(!input||!box)return;
    const q=norm(input.value);
    hits=q?(samples[lang]||samples.de).filter(x=>norm(x.q).includes(q)).slice(0,6):[];
    box.innerHTML='';active=-1;box.hidden=!hits.length;input.setAttribute('aria-expanded',String(Boolean(hits.length)));input.setAttribute('aria-activedescendant','');
    hits.forEach((item,i)=>{const b=document.createElement('button');b.type='button';b.role='option';b.id='suggestion-'+i;b.setAttribute('aria-selected','false');const text=document.createElement('span');text.textContent=item.q;const meta=document.createElement('small');meta.textContent=item.m;meta.style.marginLeft='auto';meta.style.color='#66756c';b.append(text,meta);b.addEventListener('pointerdown',e=>e.preventDefault());b.addEventListener('click',()=>choose(i));box.appendChild(b)});
  }
  function updateQuickExamples(){
    const buttons=[...document.querySelectorAll('.quick button')];
    const rows=samples[lang]||samples.de;
    buttons.forEach((button,index)=>{if(rows[index])button.textContent=rows[index].q});
  }
  function updateVoiceButton(){
    if(!voiceButton)return;
    const SpeechRecognition=window.SpeechRecognition||window.webkitSpeechRecognition;
    voiceButton.hidden=!SpeechRecognition;
    const text=t();
    voiceButton.setAttribute('aria-label',listening?text.voiceStop:text.voiceStart);
    voiceButton.title=listening?text.voiceStop:text.voiceStart;
    voiceButton.setAttribute('aria-pressed',String(listening));
  }
  function applyLanguage(next,{persist=true}={}){
    const normalized=normalizeLanguage(next)||'de';
    lang=normalized;
    const config=locales[lang]||locales.de;
    const text=t();
    document.documentElement.lang=lang;
    document.documentElement.dir=config.dir||'ltr';
    if(languageSelect)languageSelect.value=lang;
    if(input)input.placeholder=text.search||i18n.de.search;
    if(submitButton)submitButton.textContent=text.button||i18n.de.button;
  ...
  function startVoice(){
    ...
    recognition.lang=(locales[lang]||locales.de).speech;
    recognition.interimResults=false;
    recognition.maxAlternatives=1;
    recognition.onstart=()=>{listening=true;if(voiceStatus)voiceStatus.textContent=t().listening||'';updateVoiceButton()};
    recognition.onresult=event=>{const transcript=event.results?.[0]?.[0]?.transcript?.trim();if(transcript){input.value=transcript;render();input.focus()}};
    recognition.onerror=()=>{if(voiceStatus)voiceStatus.textContent=t().voiceError||i18n.de.voiceError};
    recognition.onend=()=>{listening=false;updateVoiceButton();if(voiceStatus&&voiceStatus.textContent===(t().listening||''))voiceStatus.textContent=''};
    try{recognition.start()}catch{if(voiceStatus)voiceStatus.textContent=t().voiceError||i18n.de.voiceError}
  }

  if(input){input.addEventListener('input',render);input.addEventListener('keydown',e=>{if(e.key==='ArrowDown'&&hits.length){e.preventDefault();setActive(active+1)}else if(e.key==='ArrowUp'&&hits.length){e.preventDefault();setActive(active-1)}else if(e.key==='Enter'&&active>=0){e.preventDefault();choose(active);form?.requestSubmit()}else if(e.key==='Escape')close()})}
  if(form)form.addEventListener('submit',e=>{e.preventDefault();submitSearch()});
  document.querySelectorAll('.quick button').forEach(b=>b.addEventListener('click',()=>{input.value=b.textContent;input.focus();render()}));

...
assert.ok(catalog.length>=15,'expected representative curated test catalog');
for(const product of catalog){
  for(const lang of i18n.SUPPORTED.filter(x=>x!=='en')){
    const localized=i18n.localize({title:product.name,description:product.description,brand:product.brand,family:product.family,category:product.category},lang);
...
const arabicCatalog=catalog.map(product=>i18n.localize({title:product.name,description:product.description,brand:product.brand,family:product.family,category:product.category},'ar'));
assert.ok(arabicCatalog.every(item=>/[؀-ۿ]/.test(item.description)),'Arabic catalog descriptions must contain Arabic text');
console.log('product copy localization tests passed against curated catalog');
